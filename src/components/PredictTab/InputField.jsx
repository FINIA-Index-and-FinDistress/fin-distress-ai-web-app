...
const InputField = ({ name, value, onChange, config, error, required = true, submitAttempted = false }) => {
    ...
    // Only show "Required field" when there's actually a validation error and user has attempted to submit
    const showRequiredText = required && submitAttempted && isEmpty && !error;
    ...
    return (
        <div className="group relative">
            ...
            {/* Only show "Required field" when user has attempted to submit and field is empty */}
            {showRequiredText && (
                <p className="text-xs text-red-400 mt-1 opacity-60">
                    This field is required
                </p>
