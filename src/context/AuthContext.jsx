...
const AuthContext = createContext({
    ...
    authState: {
        ...
        error: null,
        accessToken: null,
        token: null // Adding for compatibility
    },
    login: () => Promise.resolve({ success: false }),
    signIn: () => Promise.resolve({ success: false }),
    signUp: () => Promise.resolve({ success: false }),
...
export const AuthProvider = ({ children }) => {
    ...
    // FIXED: Get auth headers for API requests - now checks both accessToken state and storage
    const getAuthHeaders = useCallback(() => {
        const token = accessToken || authStorage.get(authStorage.keys.ACCESS_TOKEN);
    ...
    const signIn = useCallback(async (username, password) => {
        ...
        try {
            ...
            // FIXED: Store authentication data and update state immediately
            authStorage.set(authStorage.keys.ACCESS_TOKEN, data.access_token);
            authStorage.set(authStorage.keys.REFRESH_TOKEN, data.refresh_token || '');
            authStorage.set(authStorage.keys.USER_DATA, userData);
            authStorage.set(authStorage.keys.TOKEN_EXPIRY, tokenExpiry);
            ...
            console.log('✅ Sign in successful - token set:', !!data.access_token);
            ...
            if (typeof window !== 'undefined' && window.dispatchEvent) {
                window.dispatchEvent(new CustomEvent('auth-notification', {
                    detail: {
                        type: 'success',
                        title: 'Welcome Back!',
                        message: `Successfully signed in! Welcome to FinDistress AI.`,
                        duration: 3000
                    }
                }));
            }
    ...
    const initializeAuth = useCallback(async () => {
        try {
            ...
            // CRITICAL FIX: Token is still valid, restore session immediately and synchronously
            setUser(storedUser);
            setAccessToken(storedToken);
            setRefreshToken(storedRefreshToken || '');
            setIsAuthenticated(true);
            ...
            console.log('✅ Authentication restored from storage - token available:', !!storedToken);
    ...
    // FIXED: Create context value with proper token aliases for compatibility
    const contextValue = {
        // Core state
        user,
        isAuthenticated,
        ...
        // FIXED: Legacy compatibility for existing components - includes both token formats
        authState: {
            user,
            isAuthenticated,
            isLoading: loading,
            error,
            accessToken,
            token: accessToken, // CRITICAL: Adding token alias for backward compatibility
            refreshToken
        }
    };
