...
const usePredictionData = (dataType = 'dashboard') => {
    ...
    /**
     * CRITICAL FIX: Process backend data structure safely with correct data mapping
     */
    const processBackendData = useCallback((rawData, type) => {
        if (!rawData) return null;
        ...
        try {
            switch (type) {
                case 'analytics':
                    // CRITICAL FIX: Extract the correct data paths from your backend response
                    const keyMetrics = rawData.key_metrics || {};
                    const riskDistribution = keyMetrics.risk_distribution || [];
                    const trendAnalysis = rawData.risk_trend_analysis || [];
                    const factorContribution = rawData.factor_contribution || [];

                    console.log('🔧 FIXED Analytics Processing:');
                    console.log('  - Raw key_metrics:', keyMetrics);
                    console.log('  - Raw risk_distribution:', riskDistribution);
                    console.log('  - Raw risk_trend_analysis:', trendAnalysis);
                    console.log('  - Raw factor_contribution:', factorContribution);
                    ...
                    return {
                        ...
                        // FIXED: Direct mapping to what charts expect
                        key_metrics: {
                            total_predictions: keyMetrics.total_predictions || 0,
                            average_risk_score: keyMetrics.average_risk_score || 0,
                            risk_distribution: riskDistribution,
                            data_quality: keyMetrics.data_quality || 'Unknown',
                            health_score: keyMetrics.health_score || (100 - (keyMetrics.average_risk_score || 0) * 100)
                        },

                        // FIXED: Multiple access paths for chart components
                        riskDistribution: riskDistribution,
                        risk_distribution: riskDistribution,

                        risk_trend_analysis: trendAnalysis,
                        ...
                        peer_comparison: rawData.peer_comparison || {},
                        summary_insights: rawData.summary_insights || {},
                        dataQuality: keyMetrics.data_quality || 'Unknown',
                        lastUpdated: rawData.lastUpdated || new Date().toISOString()
                    };
    ...
    useEffect(() => {
        if (data && !data.isEmpty) {
            console.log(`[${dataType}] 📊 Data Structure Debug:`);
            console.log(`  - Has key_metrics.risk_distribution:`, !!data.key_metrics?.risk_distribution);
            console.log(`  - Risk distribution length:`, data.key_metrics?.risk_distribution?.length || 0);
            console.log(`  - Has risk_trend_analysis:`, !!data.risk_trend_analysis);
            console.log(`  - Trend analysis length:`, data.risk_trend_analysis?.length || 0);
            console.log(`  - Has factor_contribution:`, !!data.factor_contribution);
            console.log(`  - Factor contribution length:`, data.factor_contribution?.length || 0);
        }
        ...
        console.log(`[${dataType}] Hook state:`, logData);
    }, [dataType, hasData, isLoading, error?.message, dataQuality, lastFetch, authState?.isAuthenticated, data]);
