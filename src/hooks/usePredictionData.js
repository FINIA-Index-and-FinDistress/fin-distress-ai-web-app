...
// /**
//  * COMPLETELY FIXED Custom hook for fetching and managing prediction analytics data
//  * Handles authentication, retries, timeouts, and proper data processing
//  */
...
//     // FIXED: Correct API configuration with proper timeout handling
...
//     /**
//      * FIXED: Enhanced API call with proper timeout, retries, and error handling
//      */
...
//     /**
//      * FIXED: Safe data processing with comprehensive validation
//      */
...
//     /**
//      * FIXED: Process analytics data with proper validation
//      */
...
//     /**
//      * FIXED: Process insights data with proper validation
//      */
...
//     /**
//      * FIXED: Process dashboard data with proper validation
//      */
...
//     /**
//      * FIXED: Create appropriate empty data structure
//      */
...
//     /**
//      * FIXED: Main fetch function with proper endpoint mapping
//      */
...
//             }
//         }
//     }, [authState?.isAuthenticated, apiCall, dataType, data, lastFetch, isLoading, processBackendData, createEmptyData]);
...
//     // FIXED: Auto-fetch data when authentication state changes
//     useEffect(() => {
//         if (authState?.isAuthenticated) {
...
//             setError(null);
//         }
//     }, [authState?.isAuthenticated, dataType]);
...
//     /**
//      * FIXED: Enhanced API call with proper auth headers
//      */
//     const apiCall = useCallback(async (endpoint, retryCount = 0) => {
//         // Cancel previous request if still pending
//         if (abortControllerRef.current) {
...
//     /**
//      * FIXED: Main fetch function with proper authentication check
//      */
//     const fetchData = useCallback(async (forceRefresh = false) => {
...
/**
 * COMPLETE UPDATED Custom hook for fetching and managing prediction analytics data
 * Properly sends authentication headers with all requests and handles insights endpoint correctly
 */
const usePredictionData = (dataType = 'dashboard') => {
    const [data, setData] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    ...
    /**
     * FIXED: Enhanced API call with proper auth headers and better error handling
     */
    const apiCall = useCallback(async (endpoint, retryCount = 0) => {
        // Cancel previous request if still pending
        if (abortControllerRef.current) {
        ...
        try {
            ...
            // FIXED: Get auth headers properly with fallbacks
            let headers = {
                'Content-Type': 'application/json',
            ...
            if (accessToken) {
                ...
                try {
                    ...
                    console.warn(`[${dataType}] Error getting auth headers:`, authError);
                }
            }

            console.log(`[${dataType}] API call attempt ${retryCount + 1} to: ${url}`);
            console.log(`[${dataType}] Headers:`, Object.keys(headers));
            console.log(`[${dataType}] Has Authorization: ${!!headers.Authorization}`);
            ...
            console.log(`[${dataType}] Response status: ${response.status}`);
            ...
            if (!response.ok) {
                ...
                try {
                    const errorData = await response.json();
                    errorMessage = errorData.detail || errorData.message || response.statusText;
                    console.error(`[${dataType}] API Error Details:`, errorData);
                } catch (e) {
                    errorMessage = response.statusText || `HTTP Error ${response.status}`;
                }
                throw new Error(errorMessage);
            ...
            const result = await response.json();
            console.log(`[${dataType}] API Success:`, {
                endpoint,
                attempt: retryCount + 1,
                dataKeys: Object.keys(result),
                isEmpty: result.isEmpty,
                hasRecommendations: result.actionable_recommendations?.length || 0,
                hasAlerts: result.risk_alerts?.length || 0
            });
    ...
    /**
     * FIXED: Enhanced data processing with comprehensive validation
     */
    const processBackendData = useCallback((rawData, type) => {
        if (!rawData) {
            console.warn(`[${type}] No data received from backend`);
        ...
        console.log(`[${type}] Processing backend data:`, {
            dataType: type,
            keys: Object.keys(rawData),
            isEmpty: rawData.isEmpty,
            hasRecommendations: rawData.actionable_recommendations?.length || 0,
            hasAlerts: rawData.risk_alerts?.length || 0,
            hasInsightSummary: !!rawData.insight_summary
        });
    ...
    const processAnalyticsData = (rawData) => {
        ...
        return {
            isEmpty: Boolean(rawData.isEmpty),
            totalPredictions: keyMetrics.total_predictions || rawData.totalPredictions || 0,
            period_days: rawData.period_days || 30,
            date_range: rawData.date_range || {
            ...
            riskDistribution: Array.isArray(keyMetrics.risk_distribution) ? keyMetrics.risk_distribution :
                Array.isArray(rawData.riskDistribution) ? rawData.riskDistribution : [],
            risk_trend_analysis: Array.isArray(rawData.risk_trend_analysis) ? rawData.risk_trend_analysis : [],
            factor_contribution: Array.isArray(rawData.factor_contribution) ? rawData.factor_contribution : [],
            topRiskFactors: Array.isArray(rawData.factor_contribution) ? rawData.factor_contribution :
                Array.isArray(rawData.topRiskFactors) ? rawData.topRiskFactors : [],
            monthlyTrends: Array.isArray(rawData.risk_trend_analysis) ? rawData.risk_trend_analysis :
                Array.isArray(rawData.monthlyTrends) ? rawData.monthlyTrends : [],
            peer_comparison: rawData.peer_comparison || {},
            summary_insights: rawData.summary_insights || {},
            dataQuality: keyMetrics.data_quality || rawData.dataQuality || 'Unknown',
            lastUpdated: rawData.lastUpdated || new Date().toISOString()
    ...
    /**
     * FIXED: Process insights data with comprehensive validation and safety checks
     */
    const processInsightsData = (rawData) => {
        const safeProcessRecommendations = (recommendations) => {
            if (!Array.isArray(recommendations)) {
                console.warn('[processInsightsData] Recommendations is not an array:', typeof recommendations);
                return [];
            }
            ...
            return recommendations.map((rec, index) => {
                if (typeof rec === 'string') {
                    ...
                    console.warn('[processInsightsData] Invalid recommendation format:', rec);
                    return {
        ...
        const safeProcessRiskAlerts = (alerts) => {
            if (!Array.isArray(alerts)) {
                console.warn('[processInsightsData] Risk alerts is not an array:', typeof alerts);
                return [];
            }
            ...
            return alerts.map((alert, index) => {
                if (typeof alert === 'string') {
                    ...
                    return {
                        title: String(alert.title || `Risk Alert ${index + 1}`),
                        severity: String(alert.severity || 'Medium'),
                        message: String(alert.message || 'Alert message not available'),
                        impact: String(alert.impact || 'Impact assessment pending'),
                        action: String(alert.action || 'Action required'),
                    ...
                    console.warn('[processInsightsData] Invalid alert format:', alert);
                    return {
                        title: `Alert ${index + 1}`,
                        severity: 'Low',
                        message: 'Alert data format issue',
        ...
        const safeProcessMarketContext = (context) => {
            if (!Array.isArray(context)) {
                console.warn('[processInsightsData] Market context is not an array:', typeof context);
                return [];
            }
            ...
            return context.map((item, index) => {
                if (item && typeof item === 'object') {
                    return {
                        trend: String(item.trend || item.title || item.name || `Market Trend ${index + 1}`),
                        impact: String(item.impact || item.level || 'Medium'),
                        description: String(item.description || item.desc || 'Description not available'),
                        recommendation: String(item.recommendation || item.action || 'No recommendation'),
                        source: String(item.source || 'Market Analysis')
                    };
                } else {
                    return {
                        trend: String(item || `Market Trend ${index + 1}`),
                        impact: 'Medium',
                        description: 'Context data processing issue',
                        recommendation: 'Review data format',
                        source: 'Data Processing'
        ...
        const safeProcessKeyFactors = (factors) => {
            if (!Array.isArray(factors)) {
                console.warn('[processInsightsData] Key factors is not an array:', typeof factors);
                return [];
            }
            ...
            return factors.map((factor, index) => {
                if (factor && typeof factor === 'object') {
                    return {
                        factor: String(factor.factor || factor.name || `Factor ${index + 1}`),
                        impact: Number(factor.impact || 0),
                        level: String(factor.level || factor.impact_level || 'Medium'),
                        frequency: Number(factor.frequency || 0),
                        explanation: String(factor.explanation || factor.description || 'Factor analysis not available')
                    };
                } else {
                    return {
                        factor: String(factor || `Factor ${index + 1}`),
                        impact: 0,
                        level: 'Medium',
                        frequency: 0,
                        explanation: 'Factor data processing issue'
                    };
                }
            });
        };
        ...
        const rawRecommendations = rawData.actionable_recommendations || rawData.recommendations || [];
        const rawRiskAlerts = rawData.risk_alerts || rawData.riskAlerts || [];
        const rawMarketContext = rawData.market_context || rawData.marketTrends || [];
        const rawKeyFactors = rawData.key_factors_analysis || rawData.keyFactorsAnalysis || [];

        console.log('[processInsightsData] Data extraction:', {
            recommendations: rawRecommendations.length,
            alerts: rawRiskAlerts.length,
            marketContext: rawMarketContext.length,
            keyFactors: rawKeyFactors.length,
            hasInsightSummary: !!rawData.insight_summary
        });

        // Process data safely
        const processedRecommendations = safeProcessRecommendations(rawRecommendations);
        const processedAlerts = safeProcessRiskAlerts(rawRiskAlerts);
        const processedMarketContext = safeProcessMarketContext(rawMarketContext);
        const processedKeyFactors = safeProcessKeyFactors(rawKeyFactors);

        // Safe insight summary processing
        const insightSummary = rawData.insight_summary || {};
        const processedInsightSummary = {
            total_insights: Number(insightSummary.total_insights || processedRecommendations.length + processedAlerts.length),
            critical_risks: Number(insightSummary.critical_risks || processedAlerts.filter(a => a.severity === 'Critical').length),
            high_risks: Number(insightSummary.high_risks || processedAlerts.filter(a => a.severity === 'High').length),
            recommendations: Number(insightSummary.recommendations || processedRecommendations.length),
            alert_level: String(insightSummary.alert_level || 'None'),
            overall_risk_trend: String(insightSummary.overall_risk_trend || 'stable'),
            avg_risk_score: Number(insightSummary.avg_risk_score || 0),
            health_score: Number(insightSummary.health_score || 100),
            predictions_analyzed: Number(insightSummary.predictions_analyzed || 0)
        };

        const processedData = {
            isEmpty: Boolean(rawData.isEmpty),
            actionable_recommendations: processedRecommendations,
            recommendations: processedRecommendations, // Alias for compatibility
            risk_alerts: processedAlerts,
            riskAlerts: processedAlerts, // Alias for compatibility
            market_context: processedMarketContext,
            marketTrends: processedMarketContext, // Alias for compatibility
            insight_summary: processedInsightSummary,
            key_factors_analysis: processedKeyFactors,
            keyFactorsAnalysis: processedKeyFactors, // Alias for compatibility
            keyInsights: Array.isArray(rawData.key_insights) ? rawData.key_insights : [],
            dataQuality: rawData.dataQuality || 'Unknown',
            lastUpdated: rawData.lastUpdated || new Date().toISOString()
        };

        console.log('[processInsightsData] Final processed data:', {
            isEmpty: processedData.isEmpty,
            recommendations: processedData.actionable_recommendations.length,
            alerts: processedData.risk_alerts.length,
            marketContext: processedData.market_context.length,
            keyFactors: processedData.key_factors_analysis.length,
            insightSummary: processedData.insight_summary
        });
    ...
    const processDashboardData = (rawData) => {
        return {
            ...
            risk_category_breakdown: rawData.risk_category_breakdown || {
                user_distribution: [],
                benchmark_percentile: 50,
                benchmark_message: 'No benchmark data available'
            },
            key_risk_drivers: Array.isArray(rawData.key_risk_drivers) ? rawData.key_risk_drivers : [],
            trend_overview: Array.isArray(rawData.trend_overview) ? rawData.trend_overview : [],
            summary_stats: rawData.summary_stats || {
    ...
    /**
     * FIXED: Main fetch function with proper authentication check and endpoint selection
     */
    const fetchData = useCallback(async (forceRefresh = false) => {
        // FIXED: Check authentication properly
        if (!isAuthenticated) {
            console.log(`[${dataType}] Not authenticated, clearing data`);
            setData(createEmptyData(dataType));
            setError(null);
            return null;
        }
        ...
        try {
            // FIXED: Proper endpoint selection
            let endpoint;
            switch (dataType) {
                case 'analytics':
                    endpoint = '/analytics';
                    break;
                case 'insights':
                    // FIXED: Use correct insights endpoint
                    endpoint = '/insights';
                    break;
                case 'dashboard':
                default:
            ...
            // Check if request was aborted or component unmounted
            if (!result || !isMountedRef.current) {
                console.log(`[${dataType}] Request aborted or component unmounted`);
                return null;
            }
            ...
            if (isMountedRef.current) {
                ...
                setLastFetch(Date.now());
                setError(null);
                console.log(`[${dataType}] Data processed and set successfully`);
            }
            ...
            // Set empty data for better UX instead of null
            const emptyData = createEmptyData(dataType);
            setData(emptyData);
            return emptyData;
    ...
    }, [isAuthenticated, apiCall, dataType, data, lastFetch, isLoading, processBackendData, createEmptyData]);
    ...
    useEffect(() => {
        if (isAuthenticated) {
            ...
            console.log(`[${dataType}] Not authenticated - setting empty data`);
            setData(createEmptyData(dataType));
            setError(null);
        }
    }, [isAuthenticated, dataType, fetchData, createEmptyData]);
    ...
    useEffect(() => {
        const logData = {
            ...
            dataKeys: data ? Object.keys(data) : null,
            isEmpty: data?.isEmpty,
            recommendations: data?.actionable_recommendations?.length || 0,
            alerts: data?.risk_alerts?.length || 0
        };

        console.log(`[${dataType}] Hook state:`, logData);
    }, [dataType, hasData, isLoading, error, dataQuality, lastFetch, isAuthenticated, accessToken, data]);
